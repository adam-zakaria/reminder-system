    ...
    # # Queue to hold messages to be sent to the server
    ...
    #         else:
    #             logging.warning(f"Device ID {device_id} not found in device mapping.")
    ...
    #             logging.error(f"Failed to send data to server (attempt {attempt + 1}/{retries}): {e}")
    #             time.sleep(1)
    #     else:
    #         logging.error(f"Failed to send data to server after {retries} attempts.")
    ...
    #     # Start the worker thread to process messages
    ...
    #         client.disconnect()
    #         message_queue.put(None)  # Stop the worker thread
    #         worker_thread.join()
    #     except Exception as e:
...
# Load CSV data and create a device ID to description mapping
def load_device_mapping(csv_file_path):
    try:
        csv_data = pd.read_csv(csv_file_path)
        device_mapping = {}
        for index, row in csv_data.iterrows():
        ...
        logging.error(f"Error loading device mapping: {e}")
        return {}

device_mapping = load_device_mapping(CSV_FILE_PATH)
...
def process_message(data):
    ...
    if device_id in device_mapping:
        device_info = device_mapping[device_id]
        if device_info['group'] == 'microwave':  # Process only microwave data
            device_info['status'] = 'On' if data['data']['value'] != "0" else 'Off'
            device_info['components'][0]['status'] = 'Updated' if data['data']['updated'] else 'Not Updated'

            response = format_response(data, device_info)
            save_to_json(response)
            send_data_to_server(response)
        else:
            logging.info(f"Ignoring non-microwave device data: {device_id}")
    else:
        logging.warning(f"Device ID {device_id} not found in device mapping.")

def format_response(data, device_info):
    return {
        "update": {
            "home_utilities": [
                {
