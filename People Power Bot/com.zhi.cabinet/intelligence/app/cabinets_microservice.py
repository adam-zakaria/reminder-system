...
TOKEN = "BJuaLfEHHiubYf565zjwU8bKisEiQoTF"
SERVER_PATH = "https://06665d5c3529.ngrok.app"
#https://b694e948ed3a.ngrok.app/update/1234/1234/?secret_token=BJuaLfEHHiubYf565zjwU8bKisEiQoTF&status=open

# This is your microservice class, which extends the Intelligence class found in com.ppc.Bot/intelligence/intelligence.py
class CabinetsEntryMicroservice(Intelligence):
    """
    ...
    def __init__(self, botengine, parent):
        ...
        """
        resp = requests.get(f"{SERVER_PATH}/ping")
        botengine.get_logger().info(resp)
    ...
    def device_measurements_updated(self, botengine, device_object):
        ...
        botengine.get_logger().info(f"{device_object.device_id} change")
        if device_object.did_change_state(botengine):
            botengine.get_logger().info(f"{device_object.device_id} updated")
            requests.get(f"{SERVER_PATH}/update/{self.parent.location_object.location_id}/{self.parent.device_id}?secret_token={TOKEN}&status={'open' if device_object.is_open(botengine) else 'close'}")
        return
