...
const inMemoryStore = {}; // In-memory store to track sent status
...
updateEvent.on('handleActivityReminders', async (update, targetClientId) => {
  ...
  const processReminder = async (reminder, updateTimestamp) => {
    ...
    // Initialize in-memory store for the reminder with default false if it doesn't exist
    if (!inMemoryStore[reminder.id]) {
      inMemoryStore[reminder.id] = { localSent: false };
    }
    ...
    if (cronTime) {
      ...
      if (reminder.interval === 'once' && inMemoryStore[reminder.id].localSent) {
      ...
      if (reminder.interval === 'once') {
        inMemoryStore[reminder.id].localSent = true;
